...
As with explicit conversions, values of enum types (including unions of enum types and other types) _must not_ be implicitly coerced to booleans, and must instead be compared explicitly with comparison operators.
...
Identifiers _should not_ generally use `$`, except when required by naming conventions for third party frameworks. See above for more on using `$` with `Observable` values.
...
Brand new files _must_ use Google Style, regardless of the style choices of other files in the same package. When adding new code to a file that is not in Google Style, reformatting the existing code first is recommended, subject to the advice below. If this reformatting is not done, then new code _should_ be as consistent as possible with existing code in the same file, but _must not_ violate the style guide.
